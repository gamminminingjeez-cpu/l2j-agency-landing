    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <BarChart3 className="w-16 h-16 text-[#C9A962] mx-auto mb-4" />
          <h1 className="text-4xl font-bold text-white mb-4">Estadísticas</h1>
          <p className="text-[#D4C4A8]">Estadísticas en tiempo real del servidor</p>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            <motion.div
              ...
              className="p-6 rounded-2xl bg-[#2A2520]/50 border border-[#C9A962]/20 text-center"
            >
        ...
        <motion.div
          ...
          className="p-6 rounded-2xl bg-[#2A2520]/50 border border-[#C9A962]/20 mb-8"
        >
          ...
          <div className="h-64 flex items-end gap-2">
            {mockStats.onlineHistory.map((data, index) => (
              <div key={index} className="flex-1 flex flex-col items-center gap-2">
                <div
                  className="w-full bg-[#C9A962]/50 rounded-t-lg transition-all hover:bg-[#C9A962]"
                  style={{ height: `${(data.players / maxOnline) * 200}px` }}
                />
                <span className="text-xs text-[#D4C4A8]">{data.time}</span>
              </div>
            ))}
          </div>
        </motion.div>
            ...
            {mockStats.classDistribution.map((cls) => (
              <div key={cls.class}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-[#D4C4A8]">{cls.count} ({cls.percentage}%)</span>
                </div>
                <div className="h-3 bg-[#1A1714] rounded-full overflow-hidden">
                  <div
        ...
        <motion.div
          ...
          className="p-6 rounded-2xl bg-[#2A2520]/50 border border-[#C9A962]/20"
        >
          <h2 className="text-xl font-bold text-white mb-6">Distribución de Niveles</h2>
          
          <div className="grid grid-cols-5 gap-4">
            {mockStats.levelDistribution.map((level) => (
              <div key={level.range} className="text-center p-4 rounded-xl bg-[#1A1714]/50">
                <div className="text-2xl font-bold text-[#C9A962] mb-1">{level.count}</div>
                <div className="text-sm text-[#D4C4A8]">Nivel {level.range}</div>
              </div>
            ))}
          </div>
        </motion.div>
