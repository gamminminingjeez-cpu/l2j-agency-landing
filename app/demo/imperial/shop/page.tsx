    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-[#D4C4A8]">Mejora tu experiencia con items exclusivos</p>
        </div>
        ...
        {cart.length > 0 && (
          <motion.div
            ...
            className="mb-8 p-6 rounded-2xl bg-[#2A2520]/50 border border-[#C9A962]/20"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <ShoppingCart className="w-6 h-6 text-[#C9A962]" />
              ...
              <div className="flex items-center gap-4">
                ...
                <button className="px-6 py-2 rounded-lg font-semibold text-[#1A1714] bg-[#C9A962] hover:bg-[#E5D4A1]">
                  Checkout
                </button>
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredItems.map((item) => (
            ...
            >
              <div className="w-16 h-16 rounded-xl bg-[#C9A962]/20 flex items-center justify-center mb-4 mx-auto"
              >
                <ShoppingCart className="w-8 h-8 text-[#C9A962]" />
              </div>
              <h3 className="text-lg font-semibold text-white text-center mb-2">{item.name}</h3>
              <p className="text-sm text-[#D4C4A8] text-center mb-4">{item.description}</p>
              <div className="text-center mb-4">
                <span className="text-2xl font-bold text-[#C9A962]">${item.price}</span>
                <span className="text-[#D4C4A8]"> USD</span>
              </div>
              <button
                onClick={() => addToCart(item)}
                disabled={!isLoggedIn}
                className="w-full py-2 rounded-lg font-medium text-[#1A1714] bg-[#C9A962] hover:bg-[#E5D4A1] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
        ...
        {/* Payment Methods */}
        <div className="mt-12 p-6 rounded-2xl bg-[#2A2520]/50 border border-[#C9A962]/20">
