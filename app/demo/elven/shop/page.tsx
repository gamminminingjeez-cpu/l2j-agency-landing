    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-[#A0C4B0]">Mejora tu experiencia con items exclusivos</p>
        </div>
        ...
        {cart.length > 0 && (
          <motion.div
            ...
            className="mb-8 p-6 rounded-2xl bg-[#1A3A2A]/50 border border-[#27AE60]/20"
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <button className="px-6 py-2 rounded-lg font-semibold text-[#0F2918] bg-[#27AE60] hover:bg-[#2ECC71]">
                  Checkout
                </button>
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredItems.map((item) => (
            ...
            >
              <div className="w-16 h-16 rounded-xl bg-[#27AE60]/20 flex items-center justify-center mb-4 mx-auto"
              >
                <ShoppingCart className="w-8 h-8 text-[#27AE60]" />
              </div>
              <h3 className="text-lg font-semibold text-white text-center mb-2">{item.name}</h3>
              <p className="text-sm text-[#A0C4B0] text-center mb-4">{item.description}</p>
              <div className="text-center mb-4">
                <span className="text-2xl font-bold text-[#C9A962]">${item.price}</span>
                <span className="text-[#A0C4B0]"> USD</span>
              </div>
              <button
                onClick={() => addToCart(item)}
                disabled={!isLoggedIn}
                className="w-full py-2 rounded-lg font-medium text-[#0F2918] bg-[#27AE60] hover:bg-[#2ECC71] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
        ...
        {/* Payment Methods */}
        <div className="mt-12 p-6 rounded-2xl bg-[#1A3A2A]/50 border border-[#27AE60]/20">
          <h2 className="text-xl font-bold text-white mb-4 text-center">Métodos de Pago</h2>
          <div className="flex justify-center gap-8">
            <div className="flex items-center gap-2 text-[#A0C4B0]">
