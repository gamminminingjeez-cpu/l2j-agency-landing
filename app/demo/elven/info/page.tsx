    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-[#A0C4B0]">Todo lo que necesitas saber sobre L2Elven</p>
        </div>
        ...
        <motion.div
          ...
          className="p-8 rounded-2xl bg-[#1A3A2A]/50 border border-[#27AE60]/20 mb-8"
        >
          <div className="flex items-center gap-4 mb-6">
            <div className="w-16 h-16 rounded-xl bg-[#27AE60]/20 flex items-center justify-center">
              <Server className="w-8 h-8 text-[#27AE60]" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-white">{mockServerInfo.name}</h2>
              <p className="text-[#A0C4B0]">Crónica: {mockServerInfo.chronicle}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {Object.entries(mockServerInfo.rates).map(([key, value]) => (
              <div key={key} className="p-4 rounded-xl bg-[#0F2918]/50 text-center">
                <div className="text-2xl font-bold text-[#27AE60]">{value}</div>
                <div className="text-sm text-[#A0C4B0] capitalize">{key}</div>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        >
          ...
          <ul className="grid md:grid-cols-2 gap-4">
            {mockServerInfo.features.map((feature, index) => (
              <li key={index} className="flex items-center gap-3 text-[#A0C4B0]">
        ...
        <motion.div
          ...
          className="p-8 rounded-2xl bg-[#1A3A2A]/50 border border-[#E74C3C]/20"
        >
          <div className="flex items-center gap-4 mb-6">
          ...
          <ul className="space-y-3">
            {mockServerInfo.rules.map((rule, index) => (
              <li key={index} className="flex items-start gap-3 text-[#A0C4B0]">
