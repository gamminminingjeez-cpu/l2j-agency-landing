    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="text-[#A0C4B0]">Los mejores jugadores y clanes del servidor</p>
        </div>
        ...
        <motion.div
          ...
          className="rounded-2xl overflow-hidden border border-[#27AE60]/20"
        >
          {activeTab === 'pvp' && (
              ...
              <thead className="bg-[#1A3A2A]">
                <tr>
                  {['Rank', 'Nombre', 'Clase', 'Kills', 'Deaths', 'Ratio'].map((h) => (
                    <th key={h} className="px-6 py-4 text-left text-sm font-semibold text-[#A0C4B0]">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#27AE60]/10">
                {mockRankings.pvp.map((player) => (
                  <tr key={player.rank} className="bg-[#0F2918]/50 hover:bg-[#1A3A2A]/50">
              ...
              <tbody className="divide-y divide-[#27AE60]/10">
                {mockRankings.pk.map((player) => (
                  <tr key={player.rank} className="bg-[#0F2918]/50 hover:bg-[#1A3A2A]/50">
                    ...
                    <td className="px-6 py-4 text-[#A0C4B0]">{player.karma.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[#27AE60]/10">
                {mockRankings.castles.map((castle) => (
                  <tr key={castle.name} className="bg-[#0F2918]/50 hover:bg-[#1A3A2A]/50">
                    <td className="px-6 py-4 font-medium text-[#C9A962]">{castle.name}</td>
                    <td className="px-6 py-4 text-white">{castle.owner}</td>
                    <td className="px-6 py-4 text-[#A0C4B0]">{castle.leader}</td>
                    <td className="px-6 py-4 text-[#27AE60]">{castle.siegeDate}</td>
                    <td className="px-6 py-4 text-[#A0C4B0]">{castle.tax}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[#27AE60]/10">
                {mockRankings.raidBosses.map((boss) => (
                  <tr key={boss.name} className="bg-[#0F2918]/50 hover:bg-[#1A3A2A]/50">
                    ...
                    <td className="px-6 py-4 text-[#A0C4B0]">{boss.respawn}</td>
                    <td className="px-6 py-4 text-[#A0C4B0]">{boss.location}</td>
                  </tr>
                ))}
              </tbody>
            </table>
